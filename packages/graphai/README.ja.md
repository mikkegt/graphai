...
クエリテキストを埋め込みベクトルに変換する処理と、テキストチャンクを埋め込みベクトルの配列に変換する処理は、それぞれ独立しているため同時に実行できます。GraphAIはこれを自動的に認識し、並行して実行します。このような並行プログラミングは従来のプログラミングスタイルでは非常に難しいのですが、GraphAIのデータフローフローグラミングを使えば容易に実装できます。
